...
onMounted(async () => {
  ...
  listInfo.value = lists.value.filter((e) => {
    return e.is_default_favorite === 1
  })[0]
  songsinfo.list = listInfo.value
  songsinfo.likeList = listInfo.value
  result = await tokenRequest.post('/islike', { list: listInfo.value.songs })
  listInfo.value.songs = result.data
  date.value = new Date(listInfo.value.date_created)
  date.value = date.value.toLocaleDateString()
})
const PlayAll = () => {
  if (!songsinfo.likeList || songsinfo.likeList.songs.length === 0 || !songsinfo.likeList.songs) {
    ElMessage({
      type: 'warning',
      message: '你的喜欢列表是空的哦！'
    })
  ...
  songsinfo.list = songsinfo.likeList
